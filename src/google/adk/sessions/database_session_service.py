...
class DatabaseSessionService(BaseSessionService):
  ...
  async def _filter_sessions(
    ...
    async with self.database_session_factory() as sql_session:
      ...
      stmt = select(schema.StorageSession).filter(schema.StorageSession.app_name == app_name)
      
      # Filter by date range on update_time
      if start_timestamp is not None:
        start_dt = datetime.fromtimestamp(start_timestamp)
        stmt = stmt.filter(schema.StorageSession.update_time >= start_dt)
      if end_timestamp is not None:
        end_dt = datetime.fromtimestamp(end_timestamp)
        stmt = stmt.filter(schema.StorageSession.update_time <= end_dt)
      ...
      for storage_session in results:
        ...
        event_stmt = (
            select(schema.StorageEvent.timestamp, schema.StorageEvent.author)
            .filter(schema.StorageEvent.app_name == app_name)
            .filter(schema.StorageEvent.user_id == storage_session.user_id)
            .filter(schema.StorageEvent.session_id == storage_session.id)
  ...
  async def list_sessions_by_prefix(
    ...
    with self.database_session_factory() as session_factory:
      schema = self._get_schema_classes()
      results = (
          session_factory.query(schema.StorageSession)
          .filter(schema.StorageSession.app_name == app_name)
          .filter(schema.StorageSession.id.like(f"{prefix}%"))
          .order_by(schema.StorageSession.update_time.desc())
          .offset(offset)
          .limit(page_size)
          .all()
      )
  ...
  def remove_event(self, app_name: str, user_id: str, session_id: str, event_id: str) -> None:
    ...
    with self.database_session_factory() as sessionFactory:
      stmt = delete(schema.StorageEvent).where(
          schema.StorageEvent.app_name == app_name,
          schema.StorageEvent.user_id == user_id,
          schema.StorageEvent.session_id == session_id,
          schema.StorageEvent.id == event_id,
      )
  ...
  def remove_events_from_timestamp(
    ...
    with self.database_session_factory() as sessionFactory:
      ...
      stmt = delete(schema.StorageEvent).where(
          ...
          schema.StorageEvent.user_id == user_id,
          schema.StorageEvent.session_id == session_id,
          schema.StorageEvent.timestamp >= timestamp,
      )
      result = sessionFactory.execute(stmt)
      if result.rowcount == 0:
