...
class Runner:
  ...
  async def _get_or_create_session(
      self, *, user_id: str, session_id: str, config: Optional[GetSessionConfig] = None
  ) -> Session:
    ...
    session = await self.session_service.get_session(
        app_name=self.app_name, user_id=user_id, session_id=session_id, config=config
    )
    if not session:
    ...
    async def _run_with_trace(
      ...
      with tracer.start_as_current_span('invocation'):
        session = await self._get_or_create_session(
            user_id=user_id, session_id=session_id, config=session_config
        )
  ...
  def _find_agent_to_run(
    ...
    for event in filter(_event_filter, reversed(session.events)):
      ...
      if not (agent := root_agent.find_sub_agent(event.author)):
        ...
        logger.info( f'Event from an unknown agent: {event.author}, event id: {event.id}',)
        continue
